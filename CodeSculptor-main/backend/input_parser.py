...
class InputParser:
    ...
    def parse_input(text: str, algorithm_type: str) -> Dict[str, Any]:
        ...
        elif algorithm_type in ['bfs', 'dfs', 'dijkstra', 'bellman_ford']:
    ...
    def parse_array(text: str) -> Dict[str, Any]:
        ...
        if numbers:
            array = [int(n) for n in numbers[:15]]  # Limit to 15 elements for visualization
            return {'type': 'array', 'data': array, 'size': len(array)}
    ...
    def validate_array(arr: List[int], max_size: int = 15) -> Tuple[bool, str]:
        ...
        if len(arr) > max_size:
            return False, f"Array too large (max {max_size} elements)"
        return True, "Valid"
