...
def detect_algorithm_type(user_input):
    ...
    elif 'two sum' in input_lower or 'sum of two' in input_lower or ('sum' in input_lower and 'target' in input_lower) or ('sum' in input_lower and '[' in input_lower):
        return 'two_sum'
    elif 'island' in input_lower or ('grid' in input_lower and ('1' in input_lower or '0' in input_lower)):
        return 'islands'
...
END"""
        ...
        return """START Two Sum
// Two Sum Algorithm - O(n) complexity using Hash Map
// Find indices of two numbers that add up to target

1. INPUT: nums[], target
2. CREATE empty Hash Map {value: index}
3. FOR each index i and number num in nums DO
4.     complement = target - num
5.     IF complement EXISTS in map THEN
6.         RETURN [map[complement], i]
7.     END IF
8.     ADD num to map at index i
9. END FOR
10. RETURN "Not Found"
END"""

    elif algorithm_type == 'islands':
        return """START Number of Islands
// Number of Islands Algorithm - O(M*N) complexity
// Uses DFS to count connected groups of land (1s)

1. INPUT: grid[][] of size MxN
2. count = 0
3. FOR each row r from 0 to M-1 DO
4.     FOR each column c from 0 to N-1 DO
5.         IF grid[r][c] == 1 THEN
6.             count = count + 1
7.             DFS(grid, r, c) // Sink the island
8.         END IF
9.     END FOR
10. END FOR
11. RETURN count
...
FUNCTION DFS(grid, r, c):
    IF outside bounds OR grid[r][c] == 0 RETURN
    grid[r][c] = 0 // Visited
